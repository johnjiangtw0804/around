...
public class PostDumpFlow {
    ...
    private static final String PROJECT_ID = "stunning-symbol-206502";
    ...
    public static void main(String[] args) {
        ...
        // Result is Bt's form
        PCollection<Result> btRows = p.apply(Read.from(CloudBigtableIO.read(config)));      // pipeline collection bigtable, read config
        PCollection<TableRow> bqRows = btRows.apply(ParDo.of(new DoFn<Result, TableRow>() { // pipeline collection bigquery
            @Override
            public void processElement(ProcessContext c) {
                Result result = c.element();                // one row
        ...
        // relational database needs schema
        ...
        bqRows.apply(BigQueryIO.Write
                .named("Write")                     // writes function
                .to(PROJECT_ID + ":" + "post_analysis" + "." + "daily_dump_1")      // table name
                .withSchema(schema)
                .withWriteDisposition(BigQueryIO.Write.WriteDisposition.WRITE_TRUNCATE)              // configuration, form
                .withCreateDisposition(BigQueryIO.Write.CreateDisposition.CREATE_IF_NEEDED));       // configuration, create if needed
